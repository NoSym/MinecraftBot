...
const getMenu = (client: CustomClient) => {
  const menu = new MessageSelectMenu().setCustomId(name).setPlaceholder('Nothing selected')
...
const handleSelection = async (interaction: SelectMenuInteraction) => {
  ...
  const embeddedResponse = new MessageEmbed().setTitle(command.data.name).setDescription(helpText)
