...
export class CustomClient extends Client {
  ...
  constructor(options: ClientOptions) {
    ...
    if (!whitelistPath) {
      console.log('whitelist path not set')
      throw new Error('WHITELIST_PATH environment variable not set')
    }
    ...
    this.guilds.cache.forEach(async (guild) => {
      ...
      commands.forEach((command) => command.permissions.set({ permissions: this.adminPermission }))
    })
  }
